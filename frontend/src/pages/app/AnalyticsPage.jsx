                    ...
                    {days.map(day => {
                        ...
                        return (
                            <div key={day.bsKey} className="cal-day-cell" title={hTitle||undefined}>
                                <div className={cls} style={{ flexDirection:'column', gap:0, height:34, width:34, color: isToday?undefined:numColor }} aria-label={label}>
                                    <span style={{ fontSize:12, lineHeight:1, fontWeight: day.isHoliday||day.isSat||day.isSun?700:400 }}>{day.bsDay}</span>
                                    <span style={{ fontSize:8, lineHeight:1, marginTop:1, opacity: isToday?0.6:0.30 }}>{day.adDate.getDate()}</span>
                                </div>
                            </div>
                        )
                    })}
...
// ── Dashboard Assignment Modal (Updated to match image_1c6b1d.png) ───────────
function DashboardAssignmentFormModal({ assignment, courses, onClose, onSaved }) {
    ...
    // Form states updated with task_type, priority, and estimated_hours
    const [form, setForm] = useState({
        ...
        task_type:       assignment?.task_type || 'assignment', // Added default
        priority:        assignment?.priority ?? 3,              // IntegerField 1-5 in models.py
        estimated_hours: assignment?.estimated_hours ?? 1,      // Added default
    })
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            // 1. Included new fields in the API payload
        ...
        <div style={{ position:'fixed', inset:0, zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.45)', backdropFilter:'blur(2px)', padding:16 }}
            ...
            <div style={{ background:'#fff', borderRadius:16, width:'100%', maxWidth:520, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 16px 48px rgba(0,0,0,0.22)' }}
                ...
                <div style={{ padding:'20px 22px', display:'flex', flexDirection:'column', gap:14 }}>
                    ...
                    {/* 2. Added the Metadata Settings Grid layout from image_1c6b1d.png */}
...
function SubmissionPreviewModal({
    ...
    return (
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }} className="courses-calendar-row">
            ...
            .calendar-wrapper .cal-card {
                padding: 20px;
            }
            .calendar-wrapper .cal-month-title {
                font-size: 15px;
            }
            ...
            .calendar-wrapper .cal-dow {
                font-size: 11px;
            }
            .calendar-wrapper .cal-legend-item {
                font-size: 10.5px;
            }
                                        ...
                                        return (
                                            <div key={a.id} style={{ padding: '11px 12px', background: '#fff', borderRadius: 10, border: '1px solid #ece7df' }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5, flexWrap: 'wrap' }}>
                                                    <p style={{ fontSize: 12.5, fontWeight: 600, color: '#1a1f35', margin: 0, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', maxWidth: '100%', lineHeight: 1.3 }}>
                                                        {a.title}
                                                    </p>
                                                </div>
