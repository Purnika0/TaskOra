...
HOLIDAY_DATES = {
    ...
    date(2025, 10, 2), date(2025, 10, 3), date(
        2025, 10, 4), date(2025, 10, 20),
    date(2025, 10, 21), date(2025, 10, 22), date(
        2025, 10, 23), date(2025, 10, 24),
    ...
    date(2026, 10, 4), date(2026, 10, 11), date(
        2026, 10, 17), date(2026, 10, 18),
    date(2026, 10, 20), date(2026, 10, 21), date(
        2026, 10, 22), date(2026, 11, 8),
    date(2026, 11, 10), date(2026, 11, 11), date(
        2026, 11, 15), date(2026, 12, 3),
    ...
    def handle(self, *args, **options):
        ...
        for course_code, assignments in COURSE_ASSIGNMENTS.items():
            course = Course.objects.filter(
                title__startswith=course_code).first()
            if not course:
                self.stdout.write(
                    self.style.WARNING(
                        f"  ⚠ Course {course_code} not found — skipping.")
                )
                total_errors += 1
                continue
            ...
            for title, task_type, due_fn, priority, est_hours in assignments:
                ...
                existing = Assignment.objects.filter(
                    title=title, course=course).first()
        ...
        self.stdout.write(
            f"  Courses skipped (no students yet): {total_no_students}")
        self.stdout.write(
            f"  Grand total         : {total_created + total_skipped}")
        self.stdout.write("=" * 60 + "\n")
